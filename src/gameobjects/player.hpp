...
// Touch Inputs
    ...
    for (int i = 0; i < touchPositions.size(); i++) {
      ...
      if (SDL_PointInRect(&touchPos, &tmpDPadRect) && originalVel.x == 0) vel.y -= deltaSpeed;
      ...
      if (SDL_PointInRect(&touchPos, &tmpDPadRect) && originalVel.y == 0) vel.x -= deltaSpeed;
    ...
    const SDL_Rect tmpRect = {(int)(pos.x + vel.x), (int)(pos.y + vel.y), (int)size.x, (int)size.y};
